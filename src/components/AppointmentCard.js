...
const AppointmentCard = React.memo(({ item, index, onCancel }) => {
  ...
  // DỮ LIỆU MỚI: dùng specializationText hoặc specializations.join()
  const specializationText = item.specializationText || 
    ...
    {item.price ? item.price.toLocaleString('vi-VN') + ' đ' : '—'}
            ...
            <View style={styles.qrContainer}>
              <View style={styles.qrBox}>
                <QRCode value={item.id || ' '} size={64} quietZone={8} />
              </View>
...
priceText: {
  ...
  cancelGradient: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 14, gap: 8 },
